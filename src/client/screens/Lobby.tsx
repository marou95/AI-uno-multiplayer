            ...
            {players.map((p) => (
              <div key={p.sessionId} className="flex items-center justify-between bg-slate-700/50 p-3 rounded-xl border border-white/5 transition-all hover:bg-slate-700/80">
                ...
                <div className="flex items-center gap-3 min-w-0 overflow-hidden">
                  <div className={`shrink-0 w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg shadow-inner ${p.isReady ? 'bg-green-500 text-white' : 'bg-slate-600 text-slate-300'}`}>
                  ...
                  <div className="flex flex-col min-w-0">
                    <div className="text-white font-bold flex items-center gap-2">
                      ...
                      {p.sessionId === playerId && <span className="shrink-0 text-xs bg-white/20 px-1.5 py-0.5 rounded text-white/80">YOU</span>}
                    </div>
                ...
                <div className="flex items-center gap-2 shrink-0 ml-2">
                  {/* Ready Status Badge */}
                  {p.isReady ? (
                    <div className="flex items-center gap-1.5 bg-green-500/20 text-green-400 px-3 py-1.5 rounded-lg text-xs font-bold border border-green-500/30 whitespace-nowrap">
                      <CheckCircle2 size={14} /> <span className="hidden sm:inline">READY</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-1.5 bg-slate-800/50 text-slate-500 px-3 py-1.5 rounded-lg text-xs font-bold border border-slate-700 whitespace-nowrap">
                      <Loader2 size={14} className="animate-spin" /> <span className="hidden sm:inline">WAITING</span>
                    </div>
                  )}

                  {/* Bouton supprimer Bot */}
              ...
              <div key={i} className="flex items-center gap-3 bg-slate-800/30 p-3 rounded-xl border border-white/5 border-dashed opacity-50">
                ...
                {isHost && (
                  <button
                    ...
                    className="ml-auto text-xs bg-slate-700 hover:bg-slate-600 text-white px-2 py-1 rounded"
                  >
                    + BOT
                  </button>
                )}
        ...
        <div className="sticky bottom-0 z-20 bg-slate-800/90 backdrop-blur-md pt-4 pb-2 px-4 md:p-8 rounded-b-3xl border-t border-white/10">
          ...
          <div className="flex gap-3 mb-3">
            ...
            <button
              ...
              className={`flex-1 py-3 rounded-xl font-black text-base tracking-wide transition-all transform hover:scale-[1.01] active:scale-[0.99] shadow-lg flex items-center justify-center gap-2 ${me?.isReady
